...
import { PoatekCoinERC20 } from "../typechain-types/contracts/PoatekCoinERC20";

describe("PoatekCoinERC20", function () {
    let owner: string;
    let contract: PoatekCoinERC20;

    beforeEach(async () => {
        await deployments.fixture([ 'PoatekCoinERC20' ]);
        const { deployer } = await getNamedAccounts();
        owner = deployer;
        contract =  await ethers.getContract("PoatekCoinERC20") as PoatekCoinERC20;
    });
    ...
    it("Should not sign up wallet twice", async function () {
        ...
        .to.be.revertedWith("PoatekCoinERC20: Wallet already exist");
    });
    ...
    it("Should not allow to access non signed up wallets", async function () {
        ...
        .to.be.revertedWith("PoatekCoinERC20: Wallet does not exist");
    });
    ...
    it("Should not allow non-owner account to send to another account", async function () {
        ...
        .to.be.revertedWith("PoatekCoinERC20: Request can only be performed by Owner");
    });
